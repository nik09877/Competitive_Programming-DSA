...
// lets say x is the minimum no of days we need to play
// ith person can be the supervisor in x-a[i] days and so on
// so total no. of days when people are ready to be supervisors is
// x-a[1]+x-a[2]+x-a[3]+..x-a[n] = n*x-sum_of_elements ;
// if total no of days in which people are ready to be a supervisor is less than x that means they still want to play so it should be greater than x
void solve()
{
    int n, sum = 0;
    cin >> n;
