...
//     (a - b) %
//     m is equivalent to((a % m) - (b % m)) % m(a *b) % m is equivalent to((a % m) * (b % m)) % m

//     We can consider(ai — aj) %
//     m to be((ai % m) — (aj % m)) % m.So we can convert all elements of the array from a1,
//     a2, a3, ...., an to a1 % m, a2 % m, a3 % m, ...., an % m.Which means there can be atmost m different values in array.If the number of elements n is greater than m, then there must be some value which is repeated more than once(pigeon hole principal).So there will be some pair(ai, aj) where both elements are the same, hence absolute difference of that pair will be 0. Resulting in a product 0.
...
//      ai % m = aj % m means ai and aj are congruent.
