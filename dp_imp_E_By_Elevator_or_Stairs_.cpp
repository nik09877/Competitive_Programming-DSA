...
int dp[N][2], n, c, a[2][N];
// 0 -> prev choice was stairs
// 1 -> prev choice was elevator
int getAns(int floor, int prev_choice)
{
    if (floor == 1)
        return 0;

    int &ans = dp[floor][prev_choice];
    if (ans != -1)
        return ans;

    if (prev_choice == 0)
    {
        ans = a[0][floor - 1] + getAns(floor - 1, 0);
        ans = min(ans, a[1][floor - 1] + c + getAns(floor - 1, 1));
    }
    else
    {
        ans = a[0][floor - 1] + getAns(floor - 1, 0);
        ans = min(ans, a[1][floor - 1] + getAns(floor - 1, 1));
    }
    return ans;
}
void solve()
{
    cin >> n >> c;
    fo(i, 0, n) dp[i][0] = dp[i][1] = -1;
    dp[1][0] = dp[1][1] = 0;

    fo(i, 1, n - 1) cin >> a[0][i];
    fo(i, 1, n - 1) cin >> a[1][i];

    getAns(n, 0);

    // fo(i, 1, n) cerr << dp[i][0] << " " << dp[i][1] << endl;
    // fo(i, 1, n - 1) cerr << a[0][i] << " " << a[1][i] << endl;
