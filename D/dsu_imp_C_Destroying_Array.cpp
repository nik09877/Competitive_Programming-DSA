...
void solve()
{
    int n;
    cin >> n;
    vpii a(n + 1); // {value,index_is_destroyed_or_not} 0 means destroyed
    fo(i, 1, n)
    {
        cin >> a[i].ff;
        a[i].ss = 0;
    ...
    for (auto i : q)
    {
        ...
        // if both sides are activated
        if (i - 1 >= 1 and a[i - 1].ss and i + 1 <= n and a[i + 1].ss)
        {
            ds.merge(i - 1, i + 1);
            ds.merge(i - 1, i);
            int p = ds.find(i);
        ...
        else
        {
            ...
            // if no sides activated
            mx = max(mx, a[i].ff * 1ll);

            // if one side activated
            if (i - 1 >= 1 and a[i - 1].ss)
            {
                ds.merge(i - 1, i);
                int p = ds.find(i);
                mx = max(mx, ds.val[p]);
            }
            if (i + 1 <= n and a[i + 1].ss)
            {
                ds.merge(i + 1, i);
                int p = ds.find(i);
                mx = max(mx, ds.val[p]);
