...
void solve()
{
    ...
    // we can have 0 to m-1 as remainder, _ _ _ _ _ ... n spaces which denote (prefix[i] % m) and as n > m-1 , there is atleast one case where sum[0,L]%m = sum[0,R]%m which means sum[L+1,R-1]%m = 0,ie as there can be at most m-1 remainders and n places one of the remainder has to be repeated
    if (n >= m)
    {
        yes;
        return;
    ...
    // for n<m let dp[i][j] denote if it is possible to get remainder j using first i elements
    ...
    for (int i = 2; i <= n; i++)
    {
        for (int j = 0; j < m; j++)
        {
            ...
            dp[i][j] = dp[i][j] or dp[i - 1][(j - a[i - 1] + m) % m];
        }
    }
    cout << (dp[n][0] ? "YES\n" : "NO\n");
    return;
