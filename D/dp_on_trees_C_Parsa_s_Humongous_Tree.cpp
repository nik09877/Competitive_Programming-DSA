...
long long dfs(ll node, ll val, ll par)
{
    ...
    for (auto child : g[node])
    {
        if (child != par)
        {
            ll val1 = abs(val - left1[child]) + dfs(child, left1[child], node);
            ll val2 = abs(val - right1[child]) + dfs(child, right1[child], node);
            ans += max(val1, val2);
        }
    }
...
void solve()
{
    ...
    cout << max(dfs(1, left1[1], 0), dfs(1, right1[1], 0)) << "\n";
    return;
}
int32_t main()
{
