...
void solve()
{
    ...
    ll left[n+1]={0},right[n+2]={0};
    //these denote the number of moves needed to get all sheeps to ith position from left side and right side

    ll total_sheeps_seen_by_now_from_left=0;
    rep1(i,n){
        if(s[i]=='*'){
            total_sheeps_seen_by_now_from_left++;
            left[i]=left[i-1];
        }else{
            left[i]=left[i-1]+total_sheeps_seen_by_now_from_left;
        }
    }

    ll total_sheeps_seen_by_now_from_right=0;
    rrep(i,n,1){
        if(s[i]=='*'){
            total_sheeps_seen_by_now_from_right++;
            right[i]=right[i+1];
        }
        else{
            right[i]=right[i+1]+total_sheeps_seen_by_now_from_right;
        }
    }
    
    ll ans=1e11;
    ans=min({ans,right[1],left[n]});
    rep1(i,n){
