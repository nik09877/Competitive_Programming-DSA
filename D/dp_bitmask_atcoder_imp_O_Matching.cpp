...
// dp[i][mask] denotes the number of ways I can pair the ith man with all the available women in the mask

// O(n^2 * 2^n)
int getAns(int i, int mask)
{
    if (i == n)
        return 1;
    if (dp[i][mask] != -1)
    ...
    for (int j = 0; j < n; j++)
    {
        if (a[i][j] == 1 and (mask & (1 << j)))
        {
            ans = (ans + getAns(i + 1, mask ^ (1 << j))) % mod;
        }
    }
