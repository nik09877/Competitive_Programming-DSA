...
int getAns(int i, int prev_rem, int prev_taken)
{
    if (i == n)
    {
        if (prev_rem % 3 == 0 and prev_taken)
            return 1;
        return 0;
    }
    int &ans = dp[i][prev_rem][prev_taken];
    if (ans != -1)
        return ans;
    //i have not taken any previous element ie starting from this index only
    if (prev_taken == 0)
    {
        //greedy
        if (s[i] == 0)
            ans = 1 + getAns(i + 1, 0, 0);

        //2 choices end it here or continue
        else
            ans = max(((s[i] - '0') % 3 == 0) + getAns(i + 1, 0, 0), getAns(i + 1, (s[i] - '0') % 3, 1));
    }
    else
    {
        if (s[i] == 0)
            ...
            ans = max(((prev_rem + s[i] - '0') % 3 == 0) + getAns(i + 1, 0, 0), getAns(i + 1, (prev_rem + s[i] - '0') % 3, 1));
    }
    return ans;
}
void solve()
