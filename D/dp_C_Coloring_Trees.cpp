...
const int INF = 1e15;
int n, m, k, dp[101][101][101];
vi a;
vvi cost;
int go(int i, int pre_color, int grp_cnt)
{
    if (grp_cnt > k)
        return INF;
    if (i == n)
    ...
    int &ans = dp[i][pre_color][grp_cnt];
    if (ans != -1)
        return ans;
    ...
    if (a[i] == 0)
    {
        ...
        {
            if (j == pre_color)
                ans = min(ans, cost[i][j] + go(i + 1, j, grp_cnt));
            else
                ans = min(ans, cost[i][j] + go(i + 1, j, grp_cnt + 1));
        }
    }
    else
    {
        if (a[i] == pre_color)
            ans = go(i + 1, a[i], grp_cnt);
        else
            ans = go(i + 1, a[i], grp_cnt + 1);
    }
    return ans;
}
void solve()
