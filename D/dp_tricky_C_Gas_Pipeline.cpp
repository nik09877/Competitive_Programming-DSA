...
//          {
//              mpp.insert({a[i], i});
//              back.insert({i, a[i]}); //to get back original values
//              i++;
//          }
//      }
//      rep(i, n)
...
int getAns(int i, int left_pillar_height)
{
    // if s[i]=='1' that means height of left pillar and the right pillar must be of height 1,so it is not allowed to have left_pillar_height 0 if s[i]=='1'
    if (s[i] == '1' && left_pillar_height == 0)
        return 1e14;

    // this is the case when we come from height 2 to height 1 but the prev section was 1 and if prev section was one its surrounding pillars must be of height 2
    if (i >= 1 && s[i - 1] == '1' && left_pillar_height == 0)
        return 1e14;

    if (i == n)
    {
        if (left_pillar_height == 0)
            return b;
        else
            return 1e14;
    }

    int &ans = dp[i][left_pillar_height];
    if (ans != -1)
        return ans;
    if (left_pillar_height == 0)
    {
        ans = b + a + getAns(i + 1, 0);
        ans = min(ans, b + 2 * a + getAns(i + 1, 1));
    }
    else
    {
        ans = 2 * b + a + getAns(i + 1, 1);
        ans = min(ans, 2 * b + 2 * a + getAns(i + 1, 0));
    }
    return ans;
}
void solve()
