...
vpii adj[300005];
ll w[300005], dp[300005];
ll ans;
//find the maximum fuel we can have if we start from node i
...
void dfs(int node, int par)
{
    ...
    for (auto child : adj[node])
    {
        if (child.ff == par)
            continue;
        dfs(child.ff, node);
        ans = max(ans, dp[node] + dp[child.ff] - child.ss);
        dp[node] = max(dp[node], dp[child.ff] + w[node] - child.ss);
    }
}
void solve()
{
