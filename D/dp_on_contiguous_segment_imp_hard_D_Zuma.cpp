...
int getAns(int i, int j)
{
    ...
    for (int k = i + 1; k <= j; k++)
    {
        ...
        if (k == i + 1 and col[i] == col[k])
            ans = min(ans, 1 + getAns(k + 1, j));
        else if (col[i] == col[k])
            ans = min(ans, getAns(i + 1, k - 1) + getAns(k + 1, j));
        // reason for not doing +1 for curret step is we can always join col[i] with anything of length 1 in (i+1,k-1)
    }
    return ans;
}
void solve()
