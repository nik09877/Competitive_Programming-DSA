...
/*
dp[i][j] denotes the maximum answer such that the 1st substring ends at i
and 2nd substring ends at j.

if(s[i-1] == t[j-1]){
    length of lcs += 1;
    length_of_1st_substr +=1
    length_of_2nd_substr +=1
    answer += [4 * 1 - (1) - (1) = 2] //as 4*(x + y) = 4*x + 4*y
    dp[i][j] = dp[i-1][j-1] + 2;
}
else{
    dp[i][j] = max(dp[i-1][j],dp[i][j-1]) - 1
    // - 1 because we are increasing one of the substring's length
}
*/
void solve()
{
