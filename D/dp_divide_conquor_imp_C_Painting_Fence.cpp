...
int getAns(int l, int r, int pre_min_height, vi &a)
{
    if (l > r)
    {
        return 0;
    ...
    int lb = l;
    int ans = mn - pre_min_height;

    for (int ub = l; ub <= r; ++ub)
    {
        if (a[ub] == mn)
        {
            ans += getAns(lb, ub - 1, mn, a);
            lb = ub + 1;
        }
    }
    ans += getAns(lb, r, mn, a);
