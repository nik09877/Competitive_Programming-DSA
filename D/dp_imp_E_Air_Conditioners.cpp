...
void solve()
{
    ...
    rep(i, n)
    {
        left[i] = min(prev_min + 1, t[i]);
        prev_min = left[i];
    }
    int next_min = INT_MAX;
    rrep(i, n - 1, 0)
    {
        right[i] = min(next_min + 1, t[i]);
        next_min = right[i];
    }
    rep(i, n)
    {
        prsp(min(left[i], right[i]));
    }
    cout << endl;
    return;
}
