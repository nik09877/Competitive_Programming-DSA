...
void solve()
{
    ...
    n = sz(a);
    int prev_mx_prod = a[0], prev_mn_prod = a[0], ans = a[0];
    for (int i = 1; i < n; i++)
    {
        int cur_mx_prod = max({prev_mx_prod * a[i], prev_mn_prod * a[i], a[i]});
        int cur_mn_prod = min({prev_mx_prod * a[i], prev_mn_prod * a[i], a[i]});
        ans = max(ans, cur_mx_prod);
        prev_mx_prod = cur_mx_prod;
        prev_mn_prod = cur_mn_prod;
    }
    prln(ans);
    return;
}
