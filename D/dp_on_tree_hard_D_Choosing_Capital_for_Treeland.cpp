...
const int N = 2e5 + 5;
vector<vvi> g(N);
int red[N], green[N], total_red, cost[N];
// root the tree at node 1 (arbitrary)
// let red[i] denote the reverse edges in the path from 1 -> ith node
// let green[i] denote the direct edges in the path from 1 -> ith node
// let total_red denotes the total no of rev edges visited going from root
// cost[i] denotes if I made ith node capital how many edges I need to flip
void dfs(int node, int par)
{
    for (auto v : g[node])
    {
        ...
        // if I want child to be the capital calc red,green edges
        if (wt == 0)
            green[child] = green[node] + 1;
        else
            green[child] = green[node];

        if (wt == 1)
        {
            total_red++;
            red[child] = red[node] + 1;
        }
        else
            red[child] = red[node];
...
void solve()
{
    ...
    rep(i, n - 1)
    {
        int a, b;
        cin >> a >> b;
        // 0 denotes direct edge and 1 denotes reverse edge
        g[a].pb({b, 0});
        g[b].pb({a, 1});
    }
    dfs(1, -1);
    ...
    fo(i, 1, n)
    {
        cost[i] = green[i] + (total_red - red[i]);
        mnCost = min(mnCost, cost[i]);
    }
    prln(mnCost);
    fo(i, 1, n) if (cost[i] == mnCost) prsp(i);
    return;
}
