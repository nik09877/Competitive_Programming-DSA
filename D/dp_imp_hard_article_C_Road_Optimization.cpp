...
// dp[i][j] denotes currently you are at ith sign and
// you can remove at most j signs , find min time to reach from i to nth sign
int getAns(int i, int removal_left)
{
    if (i == n)
        return 0;
    int &ans = dp[i][removal_left];
    if (ans != -1)
        return ans;

    // Don't remove any sign
    ans = a[i] * (d[i + 1] - d[i]) + getAns(i + 1, removal_left);

    // remove some of the next signs
    if (removal_left)
    {
        for (int remove = 1; remove <= removal_left; remove++)
        {
            int next_sign = i + remove + 1;
            if (next_sign <= n)
            {
                ans = min(ans, a[i] * (d[next_sign] - d[i]) + getAns(next_sign, removal_left - remove));
            }
        }
    }
    return ans;
