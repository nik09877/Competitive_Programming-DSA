...
//as this is a tree we are using par but in case of a graph we have to use a visited array
...
void dfs(int node, int par)
{
    ...
    int tot_children;
    if (par == -1)
        tot_children = adj[node].size();
    else
        tot_children = adj[node].size() - 1;

    if (tot_children)
        dp[node] /= tot_children;
}
void solve()
{
