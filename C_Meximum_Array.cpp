...
/**
 * The idea is we have to maximize the mex.
 *  we need to cut the array when we know that the mex can't be bigger than the current mex.
 * mex will increase if right part of array contains an element equal to mex, we can check it using a map for right part.
...
void solve()
{
    ...
    map<ii> right;
    set<int> left;
    rep(i, n) cin >> a[i], right[a[i]]++;
    int mex = 0;
    rep(i, n)
    {
        right[a[i]]--;
        left.insert(a[i]);
        while (left.count(mex))
            mex++;

        // we can't get mex greater than this curr_mex
        if (right[mex] == 0)
        {
            ans.pb(mex);
            mex = 0;
