...
void solve()
{
    //pre[i] denotes at height i how many nodes are there that need to be destroyed
    ...
    fo(i, 1, mx) pre[i] += pre[i - 1];
    int ans = 0, prev_block_cnt = 0;
    rrep(i, mx, mn + 1)
    {
        int cur_block_cnt = pre[i];
        if (cur_block_cnt + prev_block_cnt > k)
        {
            ans++;
            prev_block_cnt = pre[i];
        }
        else
            prev_block_cnt += pre[i];
    }
