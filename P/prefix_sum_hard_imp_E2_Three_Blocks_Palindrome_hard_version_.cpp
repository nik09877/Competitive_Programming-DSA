...
/*
O((N+n)*N)
Lets say I choose a sequence [x y x],here I fixed the count of x
in the first part which in turn automatically fixed the cnt[x] in
the last part as it should be a palindrome,so we need to just find the maximum count of any number in the interval
[pos_of_last_x_in_1st_part+1 , pos_of_first_x_in_last_part-1] ie max_cnt in [l,r],using prefix sum
*/
void solve()
{
    rep(i, 200) pos[i].clear();
    ...
    rep(i, N)
    {
        if (pos[i].empty())
            continue;
        int arr_sz = pos[i].size();

        // If I consider only sequence [x x x]
        ans = max(ans, arr_sz);

        rep(j, arr_sz / 2)
        {
            int l = pos[i][j] + 1;
            int r = pos[i][arr_sz - j - 1] - 1;

            // calc max prefix sum between [l,r]
            int mx_cnt = 0;
            if (l <= r)
            {
                rep(k, N)
