...
int n;
vi a;
// dp[i][0] -> minimum hard boss killed upto ith boss if current player is my friend
// dp[i][1] -> maximum hard boss killed upto ith boss if current player is me
...
//1 based indexing
int cache[N][2];
int getAns(int i, int player)
{
    if (i > n)
        return 0;
    int &ans = cache[i][player];
    if (ans != -1)
        return ans;

    ans = 1e9;
    if (player == 0)
        ans = min(a[i] + getAns(i + 1, 1), a[i] + a[i + 1] + getAns(i + 2, 1));
    else
        ans = min(getAns(i + 1, 0), getAns(i + 2, 0));
    return ans;
}
