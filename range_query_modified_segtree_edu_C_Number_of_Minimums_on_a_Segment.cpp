...
struct segtree
{
    ...
    int size;
    vector<node> st; //st[x] holds the minimum in the range[lx,rx) and its count
    void init(int n)
    {
        size = 1;
    ...
    void build(int x, int lx, int rx, vi &a)
    {
        ...
        int m_left, cnt_left, m_right, cnt_right;
        m_left = st[2 * x + 1].mn;
        cnt_left = st[2 * x + 1].cnt;
        m_right = st[2 * x + 2].mn;
        cnt_right = st[2 * x + 2].cnt;
