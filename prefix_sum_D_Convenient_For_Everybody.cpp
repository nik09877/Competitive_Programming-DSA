...
void solve()
{
    ...
    cin >> s >> f;
    vi pre(n + 1, 0);
    rep1(i, n) pre[i] = pre[i - 1] + a[i];
    int L = s, R = f, ans = INT_MIN, timezone = -1;
    rep1(i, n)
    {
        int curr_participants;
        if (L <= R)
        {
            curr_participants = pre[R - 1] - pre[L - 1];
        }
        else
        {
            curr_participants = pre[n];
            curr_participants -= (pre[L - 1] - pre[R - 1]);
        }
        if (curr_participants > ans)
        {
            ans = curr_participants;
            timezone = i;
        }
        L--, R--;
        if (L < 1)
