...
ll bin_power(ll a, ll b, ll mod) {ll res = 1; while (b > 0) {if (b & 1)res = (res * a) % mod; a = (a * a) % mod; b = b >> 1;} return res;}
...
void solve()
{
    ...
    vi a(n);
    re(a,n);
    bool good=all_of(all(a),check);
    if(good){
        prln(0);
        return;
    }
    bool any=any_of(all(a),check);
    if(any){
        prln(1);
        return;
    }
