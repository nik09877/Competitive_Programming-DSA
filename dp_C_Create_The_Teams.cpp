...
long long n, lower_bnd;
long long a[100005], dp[100005];
long long getAns(long long i, long long member_cnt)
{
    if (i < 0)
        return 0;
    if (dp[i] != -1)
    ...
    long long x = 0, y = 0;
    //after including ith member it satisfies the condition
    if (a[i] * (member_cnt + 1) >= lower_bnd)
    {
        x = 1 + getAns(i - 1, 0);
    }
    //doesn't satisfy the condition
    y = getAns(i - 1, member_cnt + 1);
