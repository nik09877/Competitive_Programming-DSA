...
// let dp[i] denote the minimum no of flips required such that ith char is 1 and it is the last occuring 1

// while defining dp state like this there are two choices 
// 1.this is the starting and this is the ending
// 2.find the previous state and update it based on the previous state 
void solve()
{
    int n,k,ans;
    string s;
    cin >> n>>k>>s;
    vi pre(n+2),dp(n+1);
    //pre[i] denotes no of 1's upto i
    rep1(i,n){
        if(s[i-1]=='1')pre[i]=1;
        pre[i]+=pre[i-1];
    }
    ans=pre[n]; //to handle base case that if there are no 1's in the string so ans should be 0 
    // if we dont do this our ans will be 1 (and we dont want that)
    rep1(i,n){
        //this is the starting
        dp[i]=pre[i-1]+(s[i-1]=='0');
        
        if(i-k>=1){
            dp[i]=min(dp[i],dp[i-k]+pre[i-1]-pre[i-k]+(s[i-1]=='0'));
        }
