...
//for i to be the answer the numbers must be divisible by i-1 and i-2 and .. 1
// ie lcm(i-1,i-2,..,1)

//i contributes to answer (n/lcm upto i-1 -(n/lcm upto i))
// because lcm upto i-1 is a factor of lcm upto i
void solve()
{
    int N;
    cin >> N;
    ...
    fo(i, 2, 10000000)
    {
        int cur_lcm = lcm(prev_lcm, i);
        int contribution = N / prev_lcm;
        contribution -= N / cur_lcm;
        prev_lcm = cur_lcm;
        ans += contribution * i;
        ans %= mod;
        if (cur_lcm > N)
