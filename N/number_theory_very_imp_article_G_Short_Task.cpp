...
//div_sum[i] stores the sum of divisors of i
//mp[div_sum[i]] denotes minimum i whose divisor sum is c(div_sum[i])
void calc_divisors_sum()
{
    rep(i, N) mp[i] = -1;
    for (int i = 1; i < N; i++)
    {
        ...
        if (div_sum[i] < N and mp[div_sum[i]] == -1)
            mp[div_sum[i]] = i;
    }
}
void solve()
