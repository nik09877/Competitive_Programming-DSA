...
int limit;
int dp[105][1005]; // no of pair of arrays I can create of size i using n elements
int getAns(int n, int start)
{
    if (n == 0)
        return 1;
    if (start > limit)
...
void solve()
{
    ...
    int sz; //{use 1 to n and sizeof 2 array should be upto m} this is nothing but an array of size 2*m sorted in ascending order [a1,a2,..am,bm,bm-1,...,b1] find out in how many ways u can do that
    cin >> limit >> sz;
    int ans = getAns(2 * sz, 1);
    prln(ans);
    return;
}
int32_t main()
