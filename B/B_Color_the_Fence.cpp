...
void solve()
{
    ...
    sort(all(v), cmp);
    int len = totalhave / v[0].cost;
    totalhave = totalhave - v[0].cost * len;
    if (len == 0)
    {
        cout << -1 << endl;
        return;
    ...
    while (true)
    {
        int num = v[0].dig, pos = 0;
        rep1(i, 8)
        {
            if (v[i].dig > num && totalhave + v[0].cost - v[i].cost >= 0)
            {
                num = v[i].dig;
                pos = i;
            }
        ...
        {
            cout << num;
            totalhave = totalhave + v[0].cost - v[pos].cost;
            ch++;
        }
    }
    while (ch < len)
