...
bool good(int m, int &n, vi &a)
{
    ...
    rrep(i, n - 1, 2)
    {
        if (b[i] >= m)
        {
            // b[i] -> current height, a[i] -> upper bound on height
            // as the current height should not exceed the given heap size
            int extra = min(b[i] - m, a[i]);
            b[i - 1] += (extra / 3);
            b[i - 2] += (extra / 3) * 2ll;
        }
        else
