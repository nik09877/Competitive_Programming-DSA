...
bool good(int can_take, vi &arr)
{
    int total_time = abs(a - b), tempA = a, tempB = b;
    if (a < b)
        total_time += a - 1;
    else
        total_time += n - a;
    ...
    while (can_take and abs(tempA - tempB) > 1)
    {
        // debug(cur_t + arr[can_take]);

        if (cur_t + arr[can_take] > total_time)
            break;
        cur_t++;
        can_take--;
        if (a < b)
