...
/*
lets say x exists that means n xor i = x (0 <= i <= m)
=>  n xor x = i
=>  we need to find smallest number i such that n xor i >= m + 1
*/
void solve()
{
    int n, m;
    ...
    {
        //set ith bit
        if (!(n & (1 << i)) and (mask & (1 << i)))
            ans |= (1 << i), n |= (1 << i);
        //dont need to set ith bit
        if ((n & (1 << i)) and (mask & (1 << i)))
            continue;
        // break as current n ^ ans is already >= mask (ie > m)
        if ((n & (1 << i)) and !(mask & (1 << i)))
            break;
    }
    prln(ans);
