...
void solve()
{
    ...
    int want_to_add_to_make_xor_0 = 0;
    rep(i, 32)
    {
        if (Xor & (1 << i))
            want_to_add_to_make_xor_0 += (1 << i);
    }
    sum += want_to_add_to_make_xor_0;
    Xor = 0;

    cout << 2 << endl;
    cout << want_to_add_to_make_xor_0 << sp << sum << endl;
    return;
}
