...
void solve()
{
    ...
    int pre_killed = 0, pre_arrows_used = 0;
    while (q--)
    {
        int arrows;
        cin >> arrows;
        int need_to_exclude = pre_arrows_used;
        // int need_to_exclude = pre_killed > 0 ? pre[pre_killed - 1] : 0;

        // binary search
        int l = pre_killed, r = n - 1, ans = -1;
        while (l <= r)
        {
            int m = (l + r) >> 1;
            if (pre[m] - pre_arrows_used <= arrows)
            {
                ans = m;
                l = m + 1;
            }
        ...
        int cur_killed = pre_killed;
        int cur_arrows_used = pre_arrows_used + arrows;
        if (ans != -1)
        {
            cur_killed = ans + 1;
        }
        ...
        pre_killed = cur_killed;
        pre_arrows_used = cur_arrows_used;
