...
void solve()
{
    ...
    int mx = 0;
    char c1, c2;
    vector<pair<int, char>> v = {{a, 'a'}, {b, 'b'}, {c, 'c'}, {d, 'd'}};
    asort(v);
    mp['a'] = {1, 1};
    mp['b'] = {1, m};
    mp['c'] = {n, 1};
    ...
    rep(k, 4)
    {
        fo(l, k + 1, 3)
        {
            pair<int, char> a1 = v[k];
            pair<int, char> a2 = v[l];
            int dist = a1.ff + a2.ff;
            pii x = mp[a1.ss];
            pii y = mp[a2.ss];
            dist += abs(y.ff - x.ff) + abs(y.ss - x.ss);
            if (dist > mx)
            {
                mx = dist;
                c1 = a1.ss;
    ...
    cout << mp[c1].ff << sp << mp[c1].ss << sp << mp[c2].ff << sp << mp[c2].ss << endl;
    return;
}
int32_t main()
{
