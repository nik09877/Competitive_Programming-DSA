...
void solve()
{
    ...
    vpii a(n);
    vi mp(n);
    rep(i, n) cin >> a[i].ff, a[i].ss = i, mp[i] = a[i].ff;
    sort(all(a), comp);
    cin >> m;
    vector<pair<pii, int>> query(m); //{{length,pos},queryTime}
    vi ans(m);
    rep(i, m)
    {
    ...
    rep(i, m)
    {
        int lenQuery = query[i].ff.ff;
        int pos = query[i].ff.ss - 1;
        int queryTime = query[i].ss;
        while (len < lenQuery)
        {
            st.insert(a[len].ss);
            len++;
        }
        int query_ans = mp[*st.find_by_order(pos)];
        ans[queryTime] = query_ans;
    }
    rep(i, m) prln(ans[i]);
    return;
}
