...
{
    vector<vector<string>> dp(target.length() + 1, vector<string>(target.length() + 1));
    for (int i = 0; i <= target.length(); i++)
    {
        for (auto word : wordBank)
        {
            ...
            {
                vector<string> newCombination = dp[i];
                for (auto subArray : newCombination)
                {
                    // subArray.push_back(word);
                    dp[i + word.length()].push_back(word);
                }
            }
        }
    }
    vector<string> ans = dp[target.length()];
    for (auto s : ans)
    {
        cout << s << sp;
    }
