...
void solve()
{
    int n;
    cin >> n;
    vi from, to, not_decided, a(n + 1), ans(n + 1);
    rep1(i, n)
    {
        cin >> a[i];
        if (a[i] != 0)
    ...
    if (sz(not_decided) == 1)
    {
        ans[from.back()] = not_decided.back();
        ans[not_decided.back()] = to.back();
        from.pop_back(), to.pop_back(), not_decided.pop_back();
    }
    else
    {
        int Sz = not_decided.size();
        rep(i, Sz)
            ans[not_decided[i]] = not_decided[(i + 1) % Sz];
    }
    while (from.size())
    {
        ans[from.back()] = to.back();
