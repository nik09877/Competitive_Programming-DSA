...
public:
    ...
    bool getAns(int i, int j, string &s, string &p)
    {
        ...
        if (s[i] == p[j] or p[j] == '?')
            ans = getAns(i + 1, j + 1, s, p);
        else if (p[j] == '*')
        {
            //take '*' as empty sequence
            ans |= getAns(i, j + 1, s, p);

            // make '*' equal to current s[i] ,move to i+1 keeping the possibily that we might make i+1 equal to '*' also.
            ans |= getAns(i + 1, j, s, p);
        }
