...
void solve()
{
    ...
    rep1(i, n)
    {
        //if i change the ith number
        // (check if u can change the number to a[i-1]+1 and it should be less than a[i+1])

        int candidate = ((a[i - 1] + 1) < (a[i + 1])) ? a[i - 1] + 1 : -1;

        if (candidate > a[i - 1] and candidate < a[i + 1])
            ans = max(ans, 1 + pre[i - 1] + suf[i + 1]);

        // else check by increasing it to a[i-1]+1 but dont care about checking if it is less than a[i+1]
        ans = max(ans, 1 + pre[i - 1]);

        // else check by increasing it to a[i+1]-1 but dont care about checking if it is greater than a[i-1]
