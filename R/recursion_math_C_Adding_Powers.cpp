...
bool good(int n, int cnt, int &k, map<ii> &visited_power)
{
    if (n == 0)
        return true;
    if (n % k == 0)
    {
        return good(n / k, cnt + 1, k, visited_power);
    }
    if (n % k == 1)
    {
        if (visited_power[cnt])
            return 0;
        visited_power[cnt]++;
        return good((n - 1), cnt, k, visited_power);
    }
    return false;
}
void solve()
{
    ...
    vi a(n);
    re(a, n);
    map<ii> visited_power;
    int one = 0;
    rep(i, n)
    {
        if (!good(a[i], 0, k, visited_power))
        {
            no;
            return;
        }
