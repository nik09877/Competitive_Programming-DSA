...
// dp[i][j] denotes the maximum value of the given function such that we choose exactly j such segments of length m from first i elements

//Transitions
// let's consider ith element to be the ending of the jth segment
// we can take this position as the ending of the jth segment so we add dp[i-m][j-1] + sum of this segment to it or we can take the sum from previous (i-1) elements which can be divided into exactly j segments

// so dp[i][j] = max(dp[i-1][j],dp[i-m]+(pre[i]-pre[i-m]));
