...
const int INF = 1e16;
/*
int getAns(int chest_no, int depth)
{
    if (chest_no == n or depth > maxD)
        return 0;

    int &ans = dp[chest_no][depth];
    if (ans != -1)
        return ans;

    ans = 0;
    // we can always use a good key
    ans = max(ans, -k + (a[chest_no] >> depth) + getAns(chest_no + 1, depth));

    // use a bad key
    ans = max(ans, (a[chest_no] >> (depth + 1)) + getAns(chest_no + 1, depth + 1));
...
void solve()
{
    ...
    fo(depth, 1, maxD)
    {
        rep(j, n)
        {
            a[depth][j] = a[depth - 1][j] / 2;
        }
    }

    // Bottom up dp
    // Base case -> when there is only one element present
    for (int depth = 0; depth <= maxD; depth++)
    {
        // use good key
        dp[n - 1][depth] = a[depth][n - 1] - k;

        // use bad key
        if (depth < maxD)
            dp[n - 1][depth] = max(dp[n - 1][depth], a[depth + 1][n - 1]);
        else
            dp[n - 1][depth] = max(dp[n - 1][depth], 0LL);
    }
    ...
    rrep(i, n - 2, 0)
    {
        ...
        {
            // use good key
            dp[i][depth] = a[depth][i] - k + dp[i + 1][depth];

            // use bad key
            if (depth < maxD)
                dp[i][depth] = max(dp[i][depth], a[depth + 1][i] + dp[i + 1][depth + 1]);
            else
                dp[i][depth] = max(dp[i][depth], 0LL);
        }
    }
    cout << dp[0][0] << "\n";
    return;
