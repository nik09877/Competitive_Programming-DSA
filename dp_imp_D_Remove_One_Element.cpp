...
// dp[i][0] -> max len of LIS ending at i without deleting any element
// dp[i][1] -> max len of LIS ending at i by deleting 1 element
void solve()
{
    int n;
    cin >> n;
...
//   for(int i = 1;i<n-1;i++)
//   {
//     if(a[i+1]>a[i-1]) maxi = max(maxi, pre[i-1] + suf[i+1]);
//   }
