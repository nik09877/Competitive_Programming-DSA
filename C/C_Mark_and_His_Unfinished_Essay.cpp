...
struct Data
{
    int len_start, len_end, str_start, str_end;
};
...
void solve()
{
    ...
    fo(i, 1, c)
    {
        cin >> a[i].str_start >> a[i].str_end;
        a[i].len_start = a[i - 1].len_end + 1;
        a[i].len_end = a[i].len_start + (a[i].str_end - a[i].str_start);
    }
    ...
    while (q--)
    {
        ...
        {
            if (pos < a[i].len_start)
            {
                i--;
                continue;
            }
            pos = pos - a[i].len_start + a[i].str_start;
            i--;
        }
        prln(s[pos - 1]);
    }
