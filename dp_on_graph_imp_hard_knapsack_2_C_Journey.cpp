...
// similar to atcoder knapsack-2

// dp[i][j] denotes the minimum time required to reach node i using j nodes in the path

// here our required answer is a dp state because time can't be a state as space complexity will be very high
int dp[N][N];
int path[N][N];
vpii g[N];

int dfs(int node, int req_nodes)
{
    if (node == 1)
    {
        if (req_nodes == 1)
    ...
    {
        int temp_ans = child.ss + dfs(child.ff, req_nodes - 1);
        if (temp_ans < ans)
        {
            ans = temp_ans;
            path[node][req_nodes] = child.ff;
        }
    }
    return ans;
}
void solve()
{
    ...
    vi res;
    int cnt = ans, cur_node = n;
    while (cnt)
    {
        res.pb(cur_node);
        cur_node = path[cur_node][cnt];
        cnt--;
    }
    reverse(all(res));
    prln(ans);
