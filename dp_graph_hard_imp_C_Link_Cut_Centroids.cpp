...
const int N = 1e5 + 5;
vi g[N];
int subtree_sz[N], min_cc_sz[N], min_cc;
// min_cc_sz[i] denotes the minimum connected component size if ith node is removed
...
void find_min_cc_sz(int node, int par)
{
    min_cc_sz[node] = subtree_sz[1] - subtree_sz[node];
    for (auto child : g[node])
    {
        if (child == par)
            continue;
        min_cc_sz[node] = max(min_cc_sz[node], subtree_sz[child]);
        find_min_cc_sz(child, node);
    }
    min_cc = min(min_cc, min_cc_sz[node]);
}
void dfs1(int node, int par, pii &edge)
{
    if (g[node].size() == 1)
    {
        edge.ff = node, edge.ss = g[node][0];
    }
    for (auto child : g[node])
    {
        if (child == par)
            continue;
        dfs1(child, node, edge);
    }
}
void solve()
{
    ...
    int n;
    cin >> n;
    fo(i, 1, n) g[i].clear(), subtree_sz[i] = 0, min_cc_sz[i] = 0;
    rep(i, n - 1)
    {
        int x, y;
        cin >> x >> y;
    ...
    find_min_cc_sz(1, -1);
    vi centroids;
    fo(i, 1, n) if (min_cc_sz[i] == min_cc) centroids.pb(i);
    if (sz(centroids) == 1)
    {
        cout << 1 << sp << g[1].back() << endl;
        cout << 1 << sp << g[1].back() << endl;
    }
    else
    {
        int node = centroids.back();
        pair<int, int> edge = {-1, -1};
        dfs1(centroids[0], node, edge);
        cout << edge.ff << sp << edge.ss << endl;
        cout << node << sp << edge.ff << endl;
    }
    return;
}
