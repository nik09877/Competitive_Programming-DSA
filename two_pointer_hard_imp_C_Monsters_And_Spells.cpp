...
/*
Let's start from the last monster. To defeat the ith monster having health
h[i] we need to start casting spell from t[i]-h[i]+1(from_t) to t[i](to_t)

Let j = i-1 , nearest monster
here there can be 3 cases

while(i>=0){
    while(from_t[i] <= t[j]){

        CASE I
        if (from_t[i] <= from_t[j])
            jth monster will be automatically defeated
            because we start casting spell way before it is needed to defeat monster j, so j--

        CASE II
        if (from_t[j] < from_t[i])
            we have to start casting spell from from_t[j]
            so from_t = from_t[j]
            j--
    }

    Now we have CASE III where (from_t > t[j])
            so total cost of spell = 1 + 2 + 3 + ... + (to_t-from_t+1)
            total cost = x*(x+1)/2 where x = to_t - from_t + 1
            ans+=total_cost;
}

think of it as union of segments
//INTERSECTING
[1,4] U [2,6] -> [1,6]

//NOT INTERSECTING
[1,4] U [5,6] -> cost[1,4] + cost[5,6]
