...
// EDITORIAL
//dp[i] denotes what is the maximum no of beacons I can keep activated if ith beacon is activates
void solve()
{
    int n;
    cin >> n;
    ...
    rep(i, n)
    {
        int diff = a[i].ff - a[i].ss;
        int idx = lower_bound(all(a), mkp(diff, -1)) - a.begin();
        idx--;
        if (idx == -1)
            dp[i] = 1;
        else
