...
class Solution {
public:
    int par[1001]; // why 1001? becoz it is max length of accounts, see constraints (1 <= accounts.length <= 1000)
    ...
    vector<vector<string>> accountsMerge(vector<vector<string>>& accounts) {
        ...
        {
            par[i] = i; // intially we don't know anything, so every parent is unique id of itself
            ...
            for(int j = 1; j < accounts[i].size(); j++)
            {
                ...
                {
                    int parent1 = findpar(par, i);
                    int parent2 = findpar(par, email_to_id[accounts[i][j]]);
            ...
            {
                int uniq_id = findpar(par, it.second); 
