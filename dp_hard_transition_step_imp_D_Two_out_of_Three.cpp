...
// from 3 elements we can take 2 elements in 3 choose 2 ways = 3 ways
int go(int i, int pre)
{
    ...
    if (pre == 0)
    {
        ...
        ans = max(a[i], a[i + 1]) + go(i + 2, 0);
        ans = min(ans, max(a[i + 1], a[i + 2]) + go(i + 3, i));
        ans = min(ans, max(a[i], a[i + 2]) + go(i + 3, i + 1));
    }
    else
    {
        ans = max(a[pre], a[i]) + go(i + 1, 0);
        ans = min(ans, max(a[i], a[i + 1]) + go(i + 2, pre));
        ans = min(ans, max(a[pre], a[i + 1]) + go(i + 2, i));
    }
    return ans;
}
void print_ans(int i, int pre)
{
    if (i > n)
    {
        if (pre != 0)
    ...
    int ans = MAXLL, next_i = -1, next_pre = -1;
    pii cur_step;
    ...
    if (pre == 0)
    {
        ...
        int c1 = max(a[i], a[i + 1]) + go(i + 2, 0);
        int c2 = max(a[i + 1], a[i + 2]) + go(i + 3, i);
        int c3 = max(a[i], a[i + 2]) + go(i + 3, i + 1);
        ...
        if (c2 < ans)
        {
            ...
            next_i = i + 3;
            next_pre = i;
            cur_step.ff = i + 1, cur_step.ss = i + 2;
        }
        if (c3 < ans)
        {
            ans = c3;
            next_i = i + 3;
            next_pre = i + 1;
            cur_step.ff = i, cur_step.ss = i + 2;
        }
    }
    else
    {
        int c1 = max(a[pre], a[i]) + go(i + 1, 0);
        int c2 = max(a[i], a[i + 1]) + go(i + 2, pre);
        int c3 = max(a[pre], a[i + 1]) + go(i + 2, i);
        ...
        if (c3 < ans)
        {
            ...
            next_i = i + 2;
            next_pre = i;
            cur_step.ff = pre, cur_step.ss = i + 1;
        }
    }
    prsp(cur_step.ff);
    prln(cur_step.ss);
    print_ans(next_i, next_pre);
    return;
}
void solve()
{
