...
void color(int i, char &col, string &ans, int &k)
{
    int n = ans.length();
    int j = i;
    while (j < n)
...
char find_suitable_color(int req_cnt)
{
    for (int i = 0; i < 26; i++)
        if (cnt[i] >= req_cnt)
            return 'a' + i;
...
string kPeriodic(string s, int k)
{
    /*
    IMPLEMENT THIS
        let A xyz xyz A denote our answer
        left A should match right A as node i -> i+k -> i+2*k...
        here x , y , z are isolated nodes so we should have first half = second half

        calculate required pair of characters needed for middle part
        color the left and right part such that
        remaining pair of characters for the middle part is >= required pairs
    */
    int n = s.length();
    k %= n;
    string ans(n, '#');
    ...
    for (int i = 0; i < n; i++)
    {
        if (vis[i] == 0)
        {
            int req_cnt = (n - i - 1) / k + 1;
            // int req_cnt = cc_size[i];
            // cnt_cc(i, req_cnt);
            char col = find_suitable_color(req_cnt);
    ...
    string temp_ans = ans.substr(n - k, k) + ans.substr(0, n - k);
    if (temp_ans == ans)
        return ans;
    debug(ans);
    return "";
