...
void solve()
{
    ...
    lli boundary_points = 0, inside_points = 0, twiceArea = 0;
    rep(i, n)
    {
        auto pt = p[i] - p[(i + 1) % n];
        twiceArea += (p[i] * p[(i + 1) % n]);
        boundary_points += gcd(abs(pt.x), abs(pt.y));
    }
    twiceArea = abs(twiceArea);
    inside_points = (twiceArea - boundary_points + 2) / 2;
    cout << inside_points << sp << boundary_points << endl;
    return;
}
int32_t main()
{
