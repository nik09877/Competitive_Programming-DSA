...
const int N = 1e5 + 5;
int n, m, dp[N], next_node[N];
vi g[N], ans;

//dp[node] denotes the maximum distance from current node to nth node
int dfs(int node)
{
    if (node == n)
        return 0;
    ...
    {
        int child_dist = dfs(child);
        if (1 + child_dist > ans)
        {
            ans = 1 + child_dist;

            //store the path from current node to nth node
