...
// First, lets root the tree arbitrary. Let dp[i][j] represent the number of ways to color subtree i, coloring node i color j.
...
void dfs(int node, int par)
{
    ...
    for (int child : g[node])
        ...
        {
            dfs(child, node);
            dp[node][0] = mul(dp[node][0], dp[child][1]);
            dp[node][1] = mul(dp[node][1], add(dp[child][0], dp[child][1]));
        }
}
int32_t main()
{
