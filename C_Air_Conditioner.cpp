...
void solve()
{
    ...
    cin >> n >> m;
    vector<triplet> a(n);
    rep(i, n) cin >> a[i].ff >> a[i].ss.ff >> a[i].ss.ss;
    sort(all(a), comp);
    ...
    rep(i, n)
    {
        int have_time = a[i].ff - prev_time;

        int prev_L = prev_rng.ff;
        int prev_R = prev_rng.ss;

        int cur_L = a[i].ss.ff;
        int cur_R = a[i].ss.ss;

        pii cur_possible_temp_rng;

        cur_possible_temp_rng.ff = max(cur_L, prev_L - have_time);
        cur_possible_temp_rng.ss = min(cur_R, prev_R + have_time);

        int possible_L = cur_possible_temp_rng.ff;
        int possible_R = cur_possible_temp_rng.ss;

        if (cur_L > possible_R or cur_R < possible_L)
        {
            no;
            return;
        }
        prev_rng = cur_possible_temp_rng;
        prev_time = a[i].ff;
    }
    yes;
    return;
