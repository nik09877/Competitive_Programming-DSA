...
// you can also do two pointer/sliding window without taking the prefix array
bool good(int m, int &start_idx)
{
...
void solve()
{
    ...
    prln(ans);
    for (int i = start_idx; i <= start_idx + ans - 1; i++)
        a[i] = 1;
    rep(i, n)
        prsp(a[i]);
    return;
