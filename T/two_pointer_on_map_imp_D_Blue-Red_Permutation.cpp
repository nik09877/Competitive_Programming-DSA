...
void solve()
{
    ...
    auto i = b.begin();
    auto j = r.begin();
    while (i != b.end() or j != r.end())
    {
        int lb = i != b.end() ? i->ff : -1e9 + 1;
        int ub = j != r.end() ? j->ff : 1e9 + 1;
        if (need > lb and need < ub)
        {
            auto it = b.find(need);
            if (it != b.end() and it->ss > 0)
            {
                it->ss--;
                need++;
                if (it->ss == 0)
            ...
            {
                auto it2 = r.find(need);
                if (it2 != r.end() and it2->ss > 0)
                {
                    it2->ss--;
                    need++;
                    if (it2->ss == 0)
