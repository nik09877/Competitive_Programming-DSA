...
int n1, n2, n3, n4, i(0), j(0), k(0), l(0), cost = INT_MAX;
vi a, b, c, d, ans(4);
void updateAns()
{
    int diff = max({a[i - 1], b[j - 1], c[k - 1], d[l - 1]}) - min({a[i - 1], b[j - 1], c[k - 1], d[l - 1]});
    if (diff < cost)
    {
        ans[0] = a[i - 1];
        ans[1] = b[j - 1];
...
void solve()
{
    ...
    while (i < n1 || j < n2 || k < n3 || l < n4)
    {
        ...
        while (i < n1 and a[i] <= mx)
            i++;
        while (j < n2 and b[j] <= mx)
            j++;
        while (k < n3 and c[k] <= mx)
            k++;
        while (l < n4 and d[l] <= mx)
            l++;
        updateAns();
