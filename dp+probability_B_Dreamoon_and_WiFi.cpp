...
void solve()
{
    ...
    cntplus = count(all(a), '+');
    cntminus = count(all(a), '-');
    final_pos = cntplus - cntminus;

    success = getAns(0, 0);

    int freq = count(all(b), '?');
    int total_prob = 1 << freq;
    float ans = (float)success / (float)total_prob;
    pr(ans);
    return;
}
int32_t main()
