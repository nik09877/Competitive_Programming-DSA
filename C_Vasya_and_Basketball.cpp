...
void solve()
{
    ...
    asort(a);
    asort(b);
    int mx = INT_MIN, a_sc = -1, b_sc = -1;
    rep(i, n)
    {
        int cur_a = 0, cur_b = 0;
        if (upper_bound(all(a), a[i] - 1) != a.end())
        {
            int pos = upper_bound(all(a), a[i] - 1) - a.begin();
            cur_a = (n - pos) * 3 + pos * 2;
        }
        else
            cur_a = 2 * n;

        if (upper_bound(all(b), a[i] - 1) != b.end())
        {
    ...
    int cur_a, cur_b;
    if (upper_bound(all(a), b[m - 1]) != a.end())
    {
        int pos = upper_bound(all(a), b[m - 1]) - a.begin();
        cur_a = (n - pos) * 3 + pos * 2;
    }
    else
        cur_a = 2 * n;
