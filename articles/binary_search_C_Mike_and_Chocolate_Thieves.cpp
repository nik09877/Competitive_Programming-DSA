...
{
    int num_ways = 0;
    // find the number of x such that (k^3)*x <= n
    // for each k find how many ways we can fill up the first position
    // such that we have [x, k*x, k*k*x, k*k*k*x] and all are <= n
    for (int k = 2; k * k * k <= n; k++)
    {
        num_ways += (n / (k * k * k));
    }

    if (num_ways < total_ways)
        return -1;
    else if (num_ways == total_ways)
        return 0;
    else
        return 1;
}
