...
void solve()
{
    ...
    while (q--)
    {
        ...
        // find the left most node and right most node in the simple path
        int left_node = nodes[0]; // has maximum depth
        int right_node = -1;
        fo(i, 1, k - 1)
        {
            int cur_node = nodes[i];
            if (get_LCA(left_node, cur_node) != cur_node)
            {
                right_node = cur_node;
                break;
            }
        ...
        // check if it is a simple path
        // dist[left,cur_node]+dist[cur_node,right] = dist[left,right]

        int total_d = get_dist(left_node, right_node);
        bool good = true;
        for (auto i : nodes)
        {
            ...
            int left_d = get_dist(left_node, i);
            int right_d = get_dist(i, right_node);
            if (left_d + right_d != total_d)
            {
                good = false;
                break;
            }
